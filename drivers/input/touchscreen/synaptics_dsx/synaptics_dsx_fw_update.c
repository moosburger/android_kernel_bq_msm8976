...
static ssize_t fwu_sysfs_device_fw_id_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static bool boot_normal = true;
static int __init android_boot_mode(char *opt)
{
	if (!opt || !*opt)
		return 1;

	if (!strncmp(opt, "ffbm", 4)) {
		boot_normal = false;
	}
	return 1;
}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	pr_warn("Image FW ID = %d, Device FW ID = %d\n",
	...
	if (image_fw_id > device_fw_id) {
		...
		pr_info("Image firmware ID older than device firmware ID\n");
		flash_area = NONE;
		goto exit;
	}
	...
	for (ii = 0; ii < config_id_size; ii++) {
		pr_warn("Image CFG ID = %02x, Device CFG ID = %02x\n",
			fwu->img.ui_config.data[ii], fwu->config_id[ii]);
...
exit:
	fwu->device_fw_id = device_fw_id;
	if (flash_area == NONE) {
		pr_info("No need to do reflash\n");
	} else {
		if (boot_normal)
			pr_notice("Updating %s\n", flash_area == UI_FIRMWARE ?
				"UI firmware and config" : "UI config only");
		else {
			flash_area = NONE;
			pr_notice("ffbm mode, not update\n");
		}
	}
...
static void lct_tp_info(struct synaptics_rmi4_data *rmi4_data, char *module)
{
	char info[50] = {0};
	int ret;
	...
	ret = synaptics_rmi4_reg_read(rmi4_data,
		fwu->f34_fd.ctrl_base_addr, fwu->config_id, V7_CONFIG_ID_SIZE);
	if (ret < 0) {
		pr_err("read config id failed !\n");
		return;
	}

	snprintf(info, sizeof(info), "[fw]%02x%02x,[ic]S3606",
		fwu->config_id[2], fwu->config_id[3]);

	snprintf(dev_info, sizeof(dev_info), "[Vendor]%s,[fw]%02x%02x,[ic]S3606",
		module, fwu->config_id[2], fwu->config_id[3]);

	pr_notice("config id: %02x%02x%02x%02x\n",
		fwu->config_id[0],
		fwu->config_id[1],
		fwu->config_id[2], fwu->config_id[3]);
...
static int fwu_start_reflash(void)
{
	...
	fwu->config_area = PM_CONFIG_AREA;
	fwu_do_read_config();
	fwu->read_config_buf[9] = '\0';
	pr_warn("serial num: %s\n",
		fwu->read_config_buf[0] == 0 ? "null" : (char *)fwu->read_config_buf);
	...
	if (fwu->image == NULL) {
		if (!strncmp(fwu->read_config_buf, "9360-0001", 9)) {
			...
			lct_tp_info(rmi4_data, "Booyi(kotl)");
			pr_warn("Booyi kotl module, no longer update\n");
			goto exit;
		} else if (!strncmp(fwu->read_config_buf, "9360-0003", 9)) {
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
				FW_IMAGE_NAME_BOOYI, sizeof(FW_IMAGE_NAME_BOOYI),
				sizeof(FW_IMAGE_NAME_BOOYI));
			...
			pr_err("Unknown module: \n");
			lct_tp_info(rmi4_data, "unknown");
			for (i = 0; i < 10; i++)
				pr_err("config_buf[%d]: 0x%02x\n", i, fwu->read_config_buf[i]);

			pr_err("config_id: %02x%02x%02x%02x\n",
				fwu->config_id[0],
				fwu->config_id[1],
				fwu->config_id[2], fwu->config_id[3]);
			retval = -EINVAL;
	...
	if (fwu->in_bl_mode)
		pr_warn("device in bootloader mode\n");

	flash_area = fwu_go_nogo();

	if (flash_area==UI_FIRMWARE || flash_area==UI_CONFIG) {
		rmi4_data->irq_enable(rmi4_data, false, false);
	}

	if (flash_area != NONE) {
		pr_warn("enter flash program...\n");
		retval = fwu_enter_flash_prog();
		if (retval < 0) {
	...
	switch (flash_area) {
	case UI_FIRMWARE:
		pr_warn("flash firmware...\n");
		retval = fwu_do_reflash();
		rmi4_data->irq_enable(rmi4_data, true, false);
		pr_warn("flash firmware finished\n");
		rmi4_data->reset_device(rmi4_data, true);
		break;
	case UI_CONFIG:
		pr_warn("flash config...\n");
		retval = fwu_check_ui_configuration_size();
		if (retval < 0)
			break;
		fwu->config_area = UI_CONFIG_AREA;
...
exit:
	...
	pr_notice("Firmware update process finished\n");
	if (flash_area==UI_FIRMWARE || flash_area==UI_CONFIG) {
		//fwu_scan_pdt();
		lct_tp_info(rmi4_data, NULL);
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	returnval = secure_memcpy(buf, count, fwu->read_config_buf,
		...
		char *buf, loff_t pos, size_t count)
{
	...
	returnval = secure_memcpy(&fwu->ext_data_source[fwu->data_pos],
...
/*
 * invoker: synaptics_rmi4_exp_fn_work()
 */
